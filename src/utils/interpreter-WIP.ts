...
export async function sendToLLM(userPrompt: string, content: string, promptVariables: PromptVariable[], model: ModelConfig): Promise<{ userResponse: any; promptResponses: any[] }> {
    // Skip API key check for Ollama as it's locally used and does not need an API key
	...
	try {
		...
		if (model.provider === 'Anthropic') {
			...
			requestBody = {
				...
				prompt: "What is your prime directive?",
//				prompt: [
...
export async function handleInterpreterUI(
	...
	try {
		...
		const { userResponse, promptResponses } = await sendToLLM(contextToUse, contentToProcess, promptVariables, modelConfig);
